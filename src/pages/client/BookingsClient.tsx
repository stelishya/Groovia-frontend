            ...
            <div className="flex-1 pl-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center text-white/90">
                        ...
                        {request.venue && (
                            <button onClick={() => onViewMap(request.venue)} className="text-purple-300 hover:text-purple-100 underline text-xs">
                                View on Map
                            </button>
                        )}
                    </div>
            ...
            {request.status === 'accepted' && (
                <>
                    {request.paymentStatus !== 'paid' ? (
                        <button onClick={() => onPaymentClick(request)} className="bg-purple-600/50 text-white border-purple-800 hover:bg-purple-700 font-bold px-3 py-2 border rounded-md text-sm transition-colors">
                            {request.paymentStatus === 'failed' ? 'Retry Payment' : `Pay ₹${request.acceptedAmount || 0} to Confirm`}
                        </button>
                    ) : (
                        <span className="bg-green-500/20 text-green-300 font-bold px-3 py-2 rounded-md text-sm border border-green-500/50">Payment Completed</span>
                    )}
...
const BookingsPage = () => {
    ...
    const handleViewMap = async (venue: string) => {
        ...
        try {
            const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(venue)}&limit=1`);
            const data = await response.json();
            setVenueCoords(data && data.length > 0 ? [parseFloat(data[0].lat), parseFloat(data[0].lon)] : [20.5937, 78.9629]);
        } catch (error) {
    ...
    const handlePayment = (request: EventRequest) => {
        if (!request.acceptedAmount) return toast.error("No accepted amount to pay.");
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-4">
                {loading ? <p>Loading...</p> : requests.map(req => (
                    <RequestCard key={req._id} request={req} onCancelClick={(id) => { setSelectedRequestId(id); setModalOpen(true); }} onViewMap={handleViewMap} onPaymentClick={handlePayment} />
                ))}
            </div>
            <div className='flex justify-between items-center mt-8 space-x-4'>
                <span className="text-white">Page {currentPage} of {Math.max(1, Math.ceil(totalRequests / pageSize))}</span>
                <div className="flex space-x-2">
                    <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1} className="bg-purple-600 text-white px-4 py-2 rounded-lg disabled:opacity-50"><ChevronLeft /></button>
                    <button onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage >= Math.ceil(totalRequests / pageSize)} className="bg-purple-600 text-white px-4 py-2 rounded-lg disabled:opacity-50"><ChevronRight /></button>
                </div>
            </div>
            {modalOpen && <ConfirmationModal show={modalOpen} onClose={() => setModalOpen(false)} onConfirm={confirmCancel} title="Confirm Cancellation" message="Are you sure you want to cancel this booking request?" />}
            {mapModalOpen && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-purple-900 rounded-2xl p-6 max-w-3xl w-full border border-purple-500">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-2xl font-bold text-white flex items-center"><MapPin className="mr-2" /> Venue Location</h3>
                            <button onClick={() => setMapModalOpen(false)} className="text-white text-2xl">×</button>
                        </div>
                        <p className="text-purple-300 mb-4">{selectedVenue}</p>
                        {loadingCoords ? <div className="h-96 flex items-center justify-center bg-purple-800 rounded-xl"><p className="text-white">Loading...</p></div> : venueCoords && (
                            <MapContainer center={venueCoords} zoom={15} className="h-96 w-full rounded-xl border-2 border-purple-500">
                                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                                <Marker position={venueCoords}><Popup>{selectedVenue}</Popup></Marker>
                            </MapContainer>
                        )}
                    </div>
                </div>
