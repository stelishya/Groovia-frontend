...
const InstructorWorkshops = () => {
    ...
    const [workshopSortBy, setWorkshopSortBy] = useState('startDate');
    ...
    const { joinSession, isConnected, activeRoomId } = useVideoCall();
    ...
    const fetchInstructorWorkshops = useCallback(async () => {
        setLoading(true);
    ...
    const getStatus = (workshop: Workshop) => {
        ...
        return now < start ? 'upcoming' : (now <= end ? 'active' : 'completed');
    };

    const filteredInstructorWorkshops = instructorWorkshops.filter(workshop =>
        workshop.title.toLowerCase().includes(instructorSearch.toLowerCase())
    );

    return (
        <div className="flex-1 flex p-8 flex-col overflow-hidden relative text-white">
            <div className="flex border-b border-purple-700 mb-4">
                ...
                >
                    Explore Workshops ({workshops.length})
                </button>
                <button
            ...
            <div className="flex-1 overflow-y-auto pb-8 custom-scrollbar">
                {activeTab === 'instructorWorkshops' ? (
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6">
                            {filteredInstructorWorkshops.length > 0 ? (
                                filteredInstructorWorkshops
                                    ...
                                    .map((workshop) => (
                                        <InstructorWorkshopCard
                                            ...
                                            fee={workshop.fee}
                                            date={workshop.sessions[0]?.date ? new Date(workshop.sessions[0].date).toISOString().split('T')[0] : new Date(workshop.startDate).toISOString().split('T')[0]}
                                            time={workshop.sessions[0]?.startTime || '00:00'}
                                            mode={workshop.mode}
                                            ...
                                            sessionStart={workshop.sessions[0]?.date ? new Date(workshop.sessions[0].date) : new Date(workshop.startDate)}
                    ...
                    <>
                        <div className="flex justify-between items-center mb-6">
                            <div className="relative w-1/3">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search by workshop..."
                                    value={bookedSearch}
                ...
                {activeTab === 'instructorWorkshops' && filteredInstructorWorkshops.length > 0 && (
                    <UserPagination
                        current={instructorPage}
                        total={filteredInstructorWorkshops.length}
                        pageSize={pageSize}
